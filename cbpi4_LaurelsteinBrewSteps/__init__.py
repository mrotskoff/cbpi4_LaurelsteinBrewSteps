    ...
    async def run(self):
        ...
        while self.running == True:
            if self.AutoNext == True:
                await self.next()
            elif self.input_actor is not None and self.input_actor.instance.state == True:
                await self.next()
            elif last_notify >= 10:
...
class Laurelstein_MashInStep(CBPiStep):
    ...
    async def run(self):
        # TODO: turn on HERMS pump until hlt reaches target temp
    ...
    async def run(self):
        ...
        while self.running == True:
            if self.timer_expired == True:
                ...
                last_timer_warning = lastlast_timer_warningwarning + 1
...
class Laurelstein_SpargeStep(CBPiStep):
    ...
    async def run(self):
        ...
        boil_kettle_filled = False
        while self.running == True:
            if self.boil_kettle_high_float_switch.instance.state == True and boil_kettle_filled == False:
                ...
                # Alarm if specified
                await self.actor_on(self.alarm)
                boil_kettle_filled = True                
            elif boil_kettle_filled == False:
                ...
                if self.boil_kettle_low_float_switch.instance.state == True and (self.boil_kettle.instance is None or self.boil_kettle.instance.state == False):
