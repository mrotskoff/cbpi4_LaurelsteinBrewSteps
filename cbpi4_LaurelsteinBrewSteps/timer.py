...
class Timer(object):
    ...
    async def _job(self):
        ...
        self.end_time = self.start_time + self._timemout
        #self.count = self.end_time - self.start_time
        #self._timeout = self.end_time - self.start_time
        
        try:
            #while self.count > 0:
            while self._timeout > 0:
                #self.count = (self.end_time - int(time.time()))
                self._timeout = (self.end_time - int(time.time()))
    ...
    async def add(self, seconds):
        ...
        #self._timemout = self._timemout + seconds
    ...
    def get_time(self):
        return self._timemout
        
        
        
def on_timer_update(timer):
    print("time remaining: " + format_time(timer.get_time))

def on_timer_done(timer):
    print("timer done")
    
timer = Timer(60, on_update=on_timer_update, on_done=on_timer_done)
timer.start()
...
class Timer(object):
    ...
    def __init__(self, timeout_seconds, on_done = None, on_update = None) -> None:
        super().__init__()
        self.timeout = timeout
        self._timemout = self.timeout
        self._task = None
    ...
    async def add(self, seconds):
        self.end_time = self.end_time + seconds
        self._timemout = self.timemout + seconds
